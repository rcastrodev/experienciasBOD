...
$terms2 = get_terms( 'product_tag', array(
    'hide_empty' => false,
) );
...
if ( !is_wp_error( $terms2 ) ) {
    ?>
			<div class="wrslb-form-control">
				<label class="wrslb-input-label" for=<?php 
    echo  $this->input_id( 'tag' ) ;
    ?>><?php 
    _e( 'Etiquetas', WRSL_SLUG );
    ?></label>
				<select<?php 
    echo  $this->attributes( 'tag' ) ;
    ?> class="wrslb-input-select">
					<option value="0" <?php 
    echo  $this->selected( $values, 'tag', '0' ) ;
    ?>><?php 
    _e( 'All', WRSL_SLUG );
    ?></option>
					<?php 
    ...
    if ( !empty($terms2) && is_array( $terms2 ) ) {
        foreach ( $terms2 as $t ) {
            ...
            echo  $t->term_id ;
            ?>" <?php 
            echo  $this->selected( $values, 'tag', $t->term_id ) ;
            ?>><?php 
            echo  $t->name ;
            ?></option>
						<?php 
