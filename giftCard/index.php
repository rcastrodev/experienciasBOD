...

{
	...
	if($_GET['param1']=="cuantos")   // Obtener el numero de registros
	{
    	...
    	$sqlsrv = "select COUNT(*) as total from dbo.giftCards where Pais like '%".$_GET['Pais']."%'  and Ciudad like '%".$_GET['Ciudad']."%' ";
	...
	elseif($_GET["param1"]=="dame")
	{
    	...
    	$sqlsrv = "select top ".$_GET['limit']." * from (select *, ROW_NUMBER() over (order by Cantidad ) as r_n_n 
      from dbo.giftCards where Pais like '%".$_GET['Pais']."%'  and Ciudad like '%".$_GET['Ciudad']."%'  ) xx where r_n_n >=".$_GET["offset"]." ";
